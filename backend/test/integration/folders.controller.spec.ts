...
describe('FoldersController (integration)', () => {
  ...
  describe('PATCH /folders/:id', () => {
    it('should update folder name', async () => {
      ...
      expect(foldersServiceMock.update).toHaveBeenCalledWith(1, {
        name: 'Renamed Folder',
      });
    });
  ...
  describe('DELETE /folders/:id', () => {
    ...
    it('should propagate BadRequestException from service', async () => {
      ...
      expect((response.body as NestErrorResponse).message).toBe(
        'Folder not found',
      );
    });

    it('should return 400 for invalid id', async () => {
      await request(app.getHttpServer()).delete('/folders/abc').expect(400);
    });
  });
});
