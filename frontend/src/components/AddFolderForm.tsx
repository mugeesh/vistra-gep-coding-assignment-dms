...
export type AddFolderFormValues = z.infer<typeof schema>;
...
export function AddFolderForm({
  ...
  const {
    ...
    defaultValues: { name: '', parentId: defaultParentId },
  });

  useEffect(() => {
    if (isOpen) reset({ name: '', parentId: defaultParentId });
  }, [isOpen, defaultParentId, reset]);

  if (!isOpen) return null;

  const doSubmit = async (values: AddFolderFormValues) => {
    const parentId =
      values.parentId !== undefined && values.parentId !== null && !Number.isNaN(values.parentId)
        ? values.parentId
        : null;
    await onSubmit({ name: values.name.trim(), parentId });
    reset();
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <label htmlFor="folder-name" className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
              Name
            </label>
            <input
              id="folder-name"
